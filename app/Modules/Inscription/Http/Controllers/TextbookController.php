...
{
    private const TIMEZONE = 'Africa/Porto-Novo'; // GMT+1

    /**
     * Vérifie que l'utilisateur responsable a accès à la classe
     */
    ...
    /**
     * GET /responsable/classes/{classGroupId}/programs
     * Récupère les programmes d'une classe avec les infos de contrat et cahier de texte
     */
    public function getClassPrograms(Request $request, int $classGroupId)
    {
        ...
        $result = $programs->map(function (Program $program) use ($today, $now) {
            ...
            // Vérifier si l'utilisateur peut ajouter une entrée aujourd'hui
            $canAddToday = $this->canAddToday($program, $now, $contractInfo['can_add']);
            ...
            return [
                ...
                'can_add_textbook' => $canAddToday, // Changé: basé sur la vérification journalière
    ...
    /**
     * Vérifie si l'utilisateur peut ajouter une entrée aujourd'hui pour ce programme
     */
    private function canAddToday(Program $program, Carbon $now, bool $contractValid): bool
    {
        if (!$contractValid) {
            return false;
        }
        ...
        // Vérifier si on est dans la fenêtre de saisie
        return $now->gte($slotStart) && $now->lte($deadline);
    }

    /**
     * Résolution des informations de contrat
     */
    private function resolveContractInfo(?int $professorId, ?int $academicYearId, Carbon $today): array
    {
        ...
        // Contrat rejeté
        if ($contrat->status === 'rejected') {
            return ['status' => 'rejected', 'can_add' => false];
        }

        // Contrat valide (signed, ongoing, completed)
        if (in_array($contrat->status, ['signed', 'ongoing', 'completed'])) {
    ...
    public function index(Request $request, int $programId) {
        ...
        $entries->transform(function ($entry) {
            return [
                ...
                'session_date' => Carbon::parse($entry->session_date)->format('Y-m-d'),
                ...
                'homework_due_date' => $entry->homework_due_date ? Carbon::parse($entry->homework_due_date)->format('Y-m-d') : null,
    ...
    public function store(Request $request, int $programId)
    {
        ...
        // Création de l'entrée en brouillon
    ...
    /**
     * GET /responsable/programs/{programId}/can-add
     * Vérifie si l'utilisateur peut ajouter une entrée pour ce programme aujourd'hui
     */
