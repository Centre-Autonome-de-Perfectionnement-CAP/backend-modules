 ...
 */
async function notifyLaravel(event: string, payload: Record<string, unknown>): Promise<void> {
    try {
        ...
        console.warn(`[Webhook] Échec de notification Laravel pour "${event}" :`, (err as Error).message)
    }
}
...
waManager.on('qr', async (qr: string) => {
    try {
...
waManager.on('status_update', (status: { status: string }) => {
 ...
 */
async function resolveJid(sock: NonNullable<ReturnType<WhatsAppManager['getSocket']>>, cleanTo: string): Promise<string | null> {
...
app.post('/send-message', rateLimit, requireApiKey, async (req, res) => {
    const { to, text, context, module } = req.body as {
        to?: unknown; text?: unknown; context?: unknown; module?: unknown
    }
    ...
    try {
        ...
        if (!sock) {
            if (logId) await markMessageFailed(logId, 'Service WhatsApp non initialisé')
...
app.post('/send-file', rateLimit, requireApiKey, async (req, res) => {
    const { to, fileUrl, fileName, mimeType, caption, context, module, fileDisk, filePath } = req.body as {
        to?: unknown; fileUrl?: unknown; fileName?: unknown; mimeType?: unknown
        caption?: unknown; context?: unknown; module?: unknown
        fileDisk?: unknown; filePath?: unknown
    }
