...
export async function useDbAuthState(sessionId: string = 'primary-session') {

    // ─── Chargement initial des creds ─────────────────────────────────────

    async function readCreds(): Promise<any> {
        const [rows] = await pool.query<SessionRow[]>(
            'SELECT creds_iv, creds_tag, creds_data FROM wa_sessions WHERE session_id = ? LIMIT 1',
            [sessionId]
        )
        const row = rows[0]
    ...
    async function saveCreds(): Promise<void> {
        ...
        await pool.query(
            `INSERT INTO wa_sessions (session_id, creds_iv, creds_tag, creds_data, created_at, updated_at)
             VALUES (?, ?, ?, ?, NOW(), NOW())
             ON DUPLICATE KEY UPDATE
                ...
                updated_at = NOW()`,
            [sessionId, enc.iv, enc.tag, enc.data]
        )
    }

    // ─── Magasin de clés Signal (pre-keys, sessions, sender-keys...) ──────

    async function get(type: string, ids: string[]): Promise<{ [id: string]: any }> {
        const result: { [id: string]: any } = {}
        if (ids.length === 0) return result

        const placeholders = ids.map(() => '?').join(',')
        const [rows] = await pool.query<KeyRow[]>(
            `SELECT key_id, value_iv, value_tag, value_data FROM wa_session_keys
    ...
    async function set(data: { [type: string]: { [id: string]: any } }): Promise<void> {
        for (const type of Object.keys(data)) {
            for (const id of Object.keys(data[type])) {
                ...
                await pool.query(
                    `INSERT INTO wa_session_keys (session_id, key_type, key_id, value_iv, value_tag, value_data, created_at, updated_at)
                     VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
                     ON DUPLICATE KEY UPDATE
                        ...
                        updated_at = NOW()`,
                    [sessionId, type, id, enc.iv, enc.tag, enc.data]
                )
            }
        }
    }
    ...
    return {
        state: { creds, keys: { get, set } } as any,
        saveCreds,
    }
}
...
export async function updateSessionMeta(
    sessionId: string,
    meta: { phone?: string | null; displayName?: string | null; status: string; connectedAt?: Date | null }
): Promise<void> {
    await pool.query(
        `INSERT INTO wa_sessions (session_id, phone, display_name, status, connected_at, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, NOW(), NOW())
         ON DUPLICATE KEY UPDATE
            phone = VALUES(phone),
            ...
            updated_at = NOW()`,
        [sessionId, meta.phone ?? null, meta.displayName ?? null, meta.status, meta.connectedAt ?? null]
    )
}
