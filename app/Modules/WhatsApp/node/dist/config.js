...
function findLaravelEnvPath(startDir) {
    let dir = startDir;
    for (let i = 0; i < MAX_LEVELS_UP; i++) {
        ...
        if (parent === dir)
            break; // racine du filesystem atteinte
        dir = parent;
    }
    throw new Error(`[Config] Impossible de trouver le .env de backend-modules/ en remontant ` +
...
if (!config.sessionEncryptionKey)
    missing.push('WA_SESSION_ENCRYPTION_KEY');
